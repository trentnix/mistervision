...
The selected MiSTer connection was already explicitly configured with `http://192.168.1.100:32400`. Its saved server grant used that HTTP address, and the running receiver identifier matched the configured profile. The HTTP receiver-to-server connection therefore does not prevent Web from using the HTTPS PMS proxy. MiSTerVision's server URL and Web's control-request URL are selected independently.

A subsequent isolated browser comparison used the same authorized controller credential for all three routes. Same-origin HTTP PMS, cross-origin HTTPS PMS, and direct HTTP MiSTerVision polls all returned HTTP 200 and timeline XML. The identifier matched the receiver in browser JavaScript for HTTP PMS and direct HTTP MiSTerVision, but not for HTTPS PMS. These were read-only polls, not a new playback or seek test.
...
Plex Web `4.160.0` obtains LAN players from PMS's `/clients` endpoint. Its `PlexPlayerModel` retains the advertised address and port, but the request dispatcher uses the associated server unless the model receives a `baseURL` through its construction context. Adding URL attributes to the player data does not supply that context. `CloudCompanionCollection` loads providers from plex.tv's `/api/v2/companions`. Each provider's `CloudCompanionModel` loads a `CloudPlayerCollection` from `<baseURL>/resources` and supplies that provider address as the players' construction context. Web accepts those timelines without the response-identifier match required for LAN players. Sonos has provider identifier `tv.plex.sonos`. The user's working Sonos polls went to `sonos.plex.tv`, not directly to the amp or through the PMS proxy. Registering an individual device with plex.tv does not establish a cloud-provider entry. No public provider-registration process was established by this investigation.
...
Account-level registration was also tested with an isolated receiver. Plex stored the device and its local address, but Web only listed it after GDM advertisement and still polled through PMS. The resources API did not list the player, so the reported Plexamp discovery result was not reproduced. This experiment did not establish a direct Web workaround. The temporary device and credential were removed. See the [registration experiment](PLEX_REMOTE_PLAN.md#account-registration-experiment).
