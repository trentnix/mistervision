...
Jellyfin video uses progressive MPEG-2 in MPEG-TS with stereo MP3 at 48 kHz. Recorded video caps at 30 fps for NTSC or 25 fps for PAL. 480i Live TV uses 30000/1001 fps. Target assembly supplies these limits through `playback.Timing`, separately from decoder dimensions. Its zero value uses 30 fps. Current CRT targets select PAL or interlaced NTSC timing explicitly. The player does not force source speed. [Diagnostics](GO_DIAGNOSTICS.md#stream-details) separates requested transcode limits, server source metadata, and the stream properties identified by MPlayer. None of those values measures rendered frame rate.
