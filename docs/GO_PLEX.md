...
Music uses artist → album → track navigation, ordered album queues, and whole-library shuffle. Artist album lists query `/library/all?type=9&artist.id={id}` with pagination and title sorting. The generic artist-children endpoint can omit compilations, EPs, and other releases. Album tracks still use the children endpoint in server order. The complete album query is included starting with v1.6.1. Original audio files pass through the authenticated loopback proxy with byte-range support. Shared controls handle pause, track changes, and music visuals. Album queues are bounded to 10,000 rows. Shuffle requests return up to 64 tracks per batch.
